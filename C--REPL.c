...
typedef struct {
    ...
    char params[10][64];
    int paramCount;
    char body[MAX_BODY_LINES][MAX_LINE_LEN];  // Исправлено: двумерный массив
    int bodyLines;
} Function;
...
void executeLine(char *line, int *shouldBreak, int *shouldContinue) {
    ...
    if (strchr(line, '=') && !strstr(line, "==") && !strstr(line, "!=")) {
        ...
        if (getVar(name) == NULL && !isExecuting) {
            addVar(name, value, "unknown");
        } else {
            addVar(name, value, "unknown");
        }
    ...
    for (int i = 0; i < funcCount; i++) {
        if (strcmp(functions[i].name, funcName) == 0) {
            ...
            for (int j = 0; j < functions[i].bodyLines; j++) {
                ...
                strcpy(lineCopy, functions[i].body[j]);  // Исправлено: теперь body[j] это строка
...
void saveFunction(char *line) {
    ...
    while (braceCount > 0 && fgets(tempLine, sizeof(tempLine), stdin)) {
        ...
        if (strchr(tempLine, '}')) braceCount--;
        if (braceCount > 0 && strlen(tempLine) > 0 && !strchr(tempLine, '}')) {
            strcpy(f->body[f->bodyLines++], tempLine);  // Исправлено
        }
    }
...
void executeMain() {
    for (int i = 0; i < funcCount; i++) {
        if (strcmp(functions[i].name, "main") == 0) {
            ...
            for (int j = 0; j < functions[i].bodyLines; j++) {
                char lineCopy[MAX_LINE_LEN];
                strcpy(lineCopy, functions[i].body[j]);  // Исправлено
...
int main() {
    ...
    printf("\n");
    printf("╔══════════════════════════════════════════════════════════════╗\n");
    printf("║     🐺 C-- FULL REPL v1.0 - Язык программирования           ║\n");
    printf("║     Автор: Андрей (командир)                                 ║\n");
    printf("╚══════════════════════════════════════════════════════════════╝\n");
    printf("\nВведите 'help' для списка команд\n");
    printf("Пример:\n");
    printf("  fn main() {\n");
    printf("      let name = \"Андрей\"\n");
    printf("      say!(\"Привет!\", name)\n");
    printf("  }\n");
    printf("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n");
